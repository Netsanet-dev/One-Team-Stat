...
class Season(models.Model):
    ...
    league_id = models.ForeignKey(League, on_delete=models.CASCADE, related_name='leagues_season')
    start = models.DateField()
    end = models.DateField()
    current = models.BooleanField(default=False)

    def clean(self):
        # Check if the start year is not greater than end year
        if self.start >= self.end:
            raise ValidationError(f"The end date should be greater than start date.")

    def save(self, *args, **kwargs):
        today = date.today().year
        
        # Check if the current season is not greater or leass than this year.
        if self.start.year <= today <= self.end.year:
            self.current = True
        else:
            self.current = False
        
        if self.current:
            Season.objects.filter(league_id=self.league_id).exclude(id=self.id).update(current=False)
        
        self.full_clean()
        super().save(*args, **kwargs)

    def __str__(self):
        return f'League: {self.league_id.name} From {self.start} to {self.end}'    


class Club(models.Model):
    club_id = models.BigAutoField(primary_key=True)
    league_id = models.ForeignKey(League, on_delete=models.CASCADE, related_name='league_clubs')
    name = models.CharField(max_length=200)
...
class Team(models.Model):
    ...
    club_id = models.ForeignKey(Club, on_delete=models.CASCADE, related_name='team_clubs')
...
class Coach(models.Model):
    ...
    def __str__(self):
        return f'{self.first_name} {self.last_name}, Coach at {self.club_id.name}'


class Player(models.Model):
    player_id = models.BigAutoField(primary_key=True, editable=False)
    team_id = models.ForeignKey(Team, on_delete=models.CASCADE, related_name='players_team')
    ...
    def __str__(self):
        return f'{self.first_name} {self.last_name} played at {self.team_id.club_id.name}'
